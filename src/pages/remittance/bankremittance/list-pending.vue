...
const fetchData = async () => {
  try {
    ...
    const dataResponse = await makeApiCall<any[]>('GET', '/bank-remittance-pending')
...
<template>
  <VRow>
    <VCol cols="12">
      <VCard title="Pending Bank Remittance Approval">
            ...
            <tr v-for="item, index in data" v-if="data && !loading" :key="item.id">
              ...
              <td class="text-center">
                <VRow justify="end" align="center" style="flex-wrap:nowrap">
                  <a :href="`/remittance/treat-bank/${item.id}`">Proceed</a>
                </VRow>
              </td>
            </tr>
