...
account_type_id.value.value = props.accounttypes.find(item =>  item.id  == props.item.account_type_id);
...
const submit = handleSubmit((values) => {
  ...
  makeApiCall<AccountSubtype>('PUT', `/account-subtypes/${props.item.id}`, formData)
  .then((response) => {
    console.log('Account Sub Type updated successfully:', response.data); 
    emits('updated'); 
    dialog.value = false;
  })
  .catch((error) => {
              ...
              <VRow class="mxt-5 mb-5">
                <vCol cols="12" v-if=props.accounttypes>
                  <v-combobox
                    ...
                    label="Account Type"
                  ></v-combobox>
                  <!-- <selectCustom :genericOptions="props.accounttypes" :prevValue="props.item.account_group_id" :label_cus="'Group'" 
