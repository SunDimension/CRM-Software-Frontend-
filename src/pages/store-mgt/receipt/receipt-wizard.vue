...
import type { StoreItem, Store, GenericSetUp } from '@/types/globalTypes'
    ...
    newItems.forEach((item, index) => {
      // if (item.product_id !== oldItems[index]?.product_id) {
        ...
        if (selectedProduct) {
          ...
          item.quantity_stock = selectedProduct.quantity
        }
     // }
    })
  },
          ...
          <VStepperItems>
            <VStepperContent v-if="step === 1" step="1">
              ...
              <VRow v-for="(item, index) in fields" :key="index">
                ...
                <VCol cols="12" md="2">
                  <VTextField v-model="item.value.quantity_stock" label="Quantity In Stock"
                    :error-messages="errors?.[`items[${index}].quantity_stock`]" readonly />
                </VCol>
                <VCol cols="12" md="2">
              ...
              <VTable class="product-table">
                ...
                <tbody>
                  <tr v-for="(entry, index) in fields" :key="index">
                    ...
                    <td>{{ entry.value.quantity_stock }}</td>
