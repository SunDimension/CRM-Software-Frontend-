...
const fetchReceiveOrder = async () => {
  try {
    const response = await makeApiCall<any>('GET', `/price-changes/${salesOrderNo}`)
...
const fetchProducts = async () => {
  try {
    const response = await makeApiCall<any>('GET', '/create-items')
...
const saveTransaction = async () => {
  ...
  try {
    ...
    await makeApiCall<any>('POST', '/approve-price-change', payload)
    console.log('Receive Order successfully treated.')
    router.push({ path: `/store/price-print/${salesOrderNo}` })
  }
  catch (error: any) {
...
<template>
  <VContainer class="sales-receipt-page" fluid>
    ...
    <VRow class="mt-2">
      <VCol cols="12" md="9">
        <!-- Sales Receipt Info -->
        <h2>Price Adjustment Request</h2>
        ...
        <VRow class="mt-2">
          ...
          <VCol>
            <h4>Status: {{ salesOrder?.status }}</h4>
          </VCol>
        </VRow>
        ...
        <VRow class="mt-2">
          <VCol cols="3">
            <h4>Reason: {{ salesOrder?.change_reason }}</h4>
          </VCol>
          <VCol cols="3">
          ...
          <VCol cols="3">
            <h4>Date: {{ formatDate(salesOrder?.created_at) }}</h4>
          </VCol>
        </VRow>
        <!-- Product Table -->
        <VCard class="mt-4" col="12" md="6">
          ...
          <VCardText>
            <VTable>
              ...
              <tbody>
                <tr v-for="(item, index) in salesOrder?.details" :key="index">
                  <!-- <td>{{ index + 1 }}</td> -->
                  <td>{{ getItemValue(item.product_id, products) }}</td>
                  <!-- <td>{{ item.description }}</td> -->
                  <!-- <td>{{ item.quantity }}</td> -->
                  <td>{{ formatNumber(item.cost_price) }}</td>
                  <td>{{ formatNumber(item.current_selling_price) }}</td>
                  <td>{{ formatNumber(item.mark_up) }}</td>
                  <td>{{ formatNumber(item.new_selling_price) }}</td>
            ...
            <VRow>
              ...
              <VCol cols="12">
                ...
                <VRadioGroup v-model="status" row>
                  ...
                  <VRadio label="Decline" value="Declined" />
