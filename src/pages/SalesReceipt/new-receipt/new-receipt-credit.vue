...
const fetchSalesOrder = async () => {
  try {
    ...
    amountPayable.value = salesOrder.value.total_amount - salesOrder.value.total_paid
    creditBalance.value = salesOrder.value.total_paid

    const payment_type = (salesOrder.value.payment_type === 'Credit') ? 'Cash' : salesOrder.value.payment_type

    paymentType.value.push(payment_type)
    payments.value.push({ payment_type, amount: amountPayable.value, max: amountPayable.value })
    amountPaid.value = creditBalance.value
...
const saveTransaction = async () => {
  try {
    if (creditBalance.value > 0)
      payments.value.push({ payment_type: 'Previous Payment', amount: creditBalance.value, max: creditBalance.value })
...
const handlePaymentChange = (value: any) => {
  ...
  if (value.target.checked) {
    payments.value.push({ payment_type: value.target.value, amount: 0, max: totalAmount.value })
  }
  else {
    ...
    <VRow class="mt-2">
      <VCol cols="12" md="9">
        ...
        <VRow class="mt-2">
          ...
          <VCol cols="3">
            <h4>Credit Appoved: &#8358;{{ formatNumber(salesOrder?.credit_balance) }}</h4>
          </VCol>
        </VRow>
        <!-- Product Table -->
        ...
        <VCard class="mt-4">
          ...
          <VCardText>
            ...
            <VRow>
              ...
              <!-- <v-text-field label="Amount Payable" v-model="salesOrder!.total_amount" readonly></v-text-field> -->
              <VCol v-for="(item, index) in payments" :key="index" cols="12">
                <VTextField v-model="item.amount" :label="item.payment_type" type="number" :max="item.max"
              ...
              <VCol cols="12" class="form-actions">
                ...
                <VBtn :disabled="balanceAmount != 0" @click="saveTransaction">
                  Save Transaction
                </VBtn>
                <!-- </div> -->
              </VCol>
