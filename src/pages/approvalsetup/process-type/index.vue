...
const data = ref<ApprovalProcessType[]>([]);
const loading = ref(true);

const fetchData = async () => {
  try {
    loading.value = true;
    const response = await makeApiCall<ApprovalProcessType[]>('GET', '/approval-process-types');
...
<template>
  <VRow>
    <VCol cols="12">
      <VCard title="Approval Process Types">
            ...
            <tr v-if="data && !loading" v-for="item in data" :key="item.id">
              ...
              <td class="text-center">
                <VRow justify="end" align="center" style="flex-wrap:nowrap">
                  ...
                  <DeleteBtn :api="`approval-process-types/${item.id}`" @deleted="handleUpdated"/>
                </VRow>
              </td>
            </tr>
